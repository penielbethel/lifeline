...
app.post('/api/auth/register-admin', async (req, res) => {
    const { username, password, tokenCode, role } = req.body;
    ...
    try {
        ...
        let validRole = role ? role.toLowerCase() : 'admin';
        const allowedRoles = ['admin', 'doctor', 'nurse', 'lab_scientist', 'pharmacist', 'hr'];
        if (!allowedRoles.includes(validRole)) validRole = 'admin';
...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        const user = await User.findOne({ username, role: { $ne: 'superadmin' } });
        if (!user) {
        ...
        await logActivity(user._id, user.username, 'LOGIN', `${user.role} logged in`);
